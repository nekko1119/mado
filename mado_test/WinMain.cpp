...
int __stdcall WinMain(HINSTANCE, HINSTANCE, LPSTR, int) try {
    ...
    button->set_on_create_handler([](mado::button&) {
        MessageBox(nullptr, _T("bar"), _T("bar"), MB_OK);
    });
    button->create();
    button->set_on_click_handler([](mado::button&, std::pair<short, short>) {
        MessageBox(nullptr, _T("clicked"), _T("button"), MB_OK);
    });
    form->title(_T("foo"));
    form->show();
